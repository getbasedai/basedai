...
def _create_seal_hash(block_and_computekey_hash_bytes: bytes, nonce: int) -> bytes:
    nonce_bytes = binascii.hexlify(nonce.to_bytes(8, "little"))
    pre_seal = nonce_bytes + binascii.hexlify(block_and_computekey_hash_bytes)[:64]
...
class POWSolution:
    ...
    def is_stale(self, basednode: "basedai.basednode") -> bool:
        """Returns True if the POW is stale.
        This means the block the POW is solved for is within 3 blocks of the current block.
        """
        return self.block_number < basednode.get_current_block() - 3
...
class _Solver(_SolverBase):
    def run(self):
        block_number: int
        block_and_computekey_hash_bytes: bytes
        ...
        while not self.stopEvent.is_set():
            if self.newBlockEvent.is_set():
                with self.check_block:
                    ...
                    block_and_computekey_hash_bytes = bytes(self.curr_block)
            ...
            solution = _solve_for_nonce_block(
                nonce_start,
                nonce_end,
                block_and_computekey_hash_bytes,
                block_difficulty,
                self.limit,
                block_number,
            )
...
class _CUDASolver(_SolverBase):
    ...
    def run(self):
        ...
        block_and_computekey_hash_bytes: bytes = b"0" * 32  # dummy value
...
def _solve_for_nonce_block(
    ...
    for nonce in range(nonce_start, nonce_end):
        # Create seal.
        seal = _create_seal_hash(block_and_computekey_hash_bytes, nonce)
...
def _hash_block_with_computekey(block_bytes: bytes, computekey_bytes: bytes) -> bytes:
    """Hashes the block with the computekey using Keccak-256 to get 32 bytes"""
    kec = keccak.new(digest_bits=256)
    kec = kec.update(bytearray(block_bytes + computekey_bytes))
    block_and_computekey_hash_bytes = kec.digest()
    return block_and_computekey_hash_bytes
...
class RegistrationStatisticsLogger:
    ...
    def __init__(
        ...
        if output_in_place:
            self.status = self.console.status("Searching for solution...")
        else:
            self.status = None
    ...
    def get_status_message(
        ...
        message = (
            "Mining...\n"
            + f"Elapsed (total): [bold white]{timedelta(seconds=stats.time_spent_total)}[/bold white]\n"
            + (
                f"Elapsed Mining: {timedelta(seconds=stats.time_spent)}\n"
                + f"Elapsed Average: {timedelta(seconds=stats.time_average)}\n"
                if verbose
                else ""
            )
            + f"Latest Block Hash: [bold white]{stats.block_hash.encode('utf-8')}[/bold white]\n"
            + f"Block Height: [bold white]{stats.block_number}[/bold white]\n"
            + f"Local Performance: [bold white]{get_human_readable(stats.hash_rate_perpetual, 'H')}/s[/bold white]\n"
            + f"BasedAI Faucet Weight: [bold white]{millify(stats.difficulty)}[/bold white] (~5-25m)\n"
        )
        return message
...
def _solve_for_difficulty_fast(
    ...
    """
    Solves the POW for memorization using multiprocessing.
    Args:
        basednode
            Basednode to connect to for block information and to submit.
        wallet:
            wallet to use for memorization.
        netuid: int
            The netuid of the Brain for the memorize operation.
    ...
    block_number, difficulty, block_hash = _get_block_with_retry(
        basednode=basednode, netuid=netuid
    )
    ...
    while netuid == -1 or not basednode.is_computekey_registered(
        netuid=netuid, computekey_ss58="0x" + wallet.computekey.public_key.hex()[-40:]
    ):
        ...
        old_block_number = _check_for_newest_block_and_update(
            basednode=basednode,
            netuid=netuid,
            computekey_bytes=computekey_bytes,
...
def _get_block_with_retry(
    basednode: "basedai.basednode", netuid: int
) -> Tuple[int, int, bytes]:
    """
    Gets the current block number, difficulty, and block hash from the Brain node.

    Args:
        basednode (:obj:`basedai.basednode`, `required`):
            The basednode object to use to get the block number, difficulty, and block hash.
    ...
    Returns:
        ...
        difficulty (:obj:`int`):
            The current difficulty of the brain (if this has been configured by the Brain owner).
    ...
    """
    block_number = basednode.get_current_block()
...
def _check_for_newest_block_and_update(
    ...
    Args:
        basednode (:obj:`basedai.basednode`, `required`):
            The basednode object to use for getting the current block.
        netuid (:obj:`int`, `required`):
            The netuid to use for retrieving the difficulty.
        old_block_number (:obj:`int`, `required`):
            The old block number to check against.
        computekey_bytes (:obj:`bytes`, `required`):
            The bytes of the computekey's pubkey.
...
def _solve_for_difficulty_fast_cuda(
    ...
    Args:
        basednode: basedai.basednode
            The basednode node to grab blocks
        wallet: basedai.wallet
        ...
        dev_id: Union[List[int], int]
            The CUDA device IDs to execute the memorization on, either a single device or a list of devices
        n_samples: int
            The number of samples of the hash_rate to keep for the EWMA
        alpha_: float
            The alpha for the EWMA for the hash_rate calculation
    ...
    with _UsingSpawnStartMethod(force=True):
        ...
        computekey_bytes = wallet.computekey.public_key
        # Start workers
        solvers = [
            _CUDASolver(
                i,
...
def create_pow(
    ...
    """
    Creates a proof of work for the given basednode and wallet.
    Args:
        basednode (:obj:`basedai.basednode.basednode`, `required`):
            The basednode to create a proof of work for.
        wallet (:obj:`basedai.wallet.wallet`, `required`):
            The wallet to create a proof of work for.
        netuid (:obj:`int`, `required`):
            The unique Brain ID of the Brain to create a proof of work for.
    ...
    Raises:
        :obj:`ValueError`: If the Brain does not exist.
    """
    if netuid != -1:
        if not basednode.subnet_exists(netuid=netuid):
            raise ValueError(f"Brain {netuid} does not exist")

...
def solve_cuda(
    ...
    Args:
        ...
        tpb: int
            Threads per block.
        block_and_computekey_hash_bytes: bytes
            Keccak(Bytes of the block hash + bytes of the computekey) 64 bytes.
        difficulty: int256
    ...
    def _create_seal_hash(block_and_computekey_hash_hex: bytes, nonce: int) -> bytes:
        nonce_bytes = binascii.hexlify(nonce.to_bytes(8, "little"))
        pre_seal = nonce_bytes + block_and_computekey_hash_hex
    ...
    solution = cubit.solve_cuda(
        ...
        update_interval,
        upper_bytes,
        block_and_computekey_hash_hex,
        dev_id,
    )  # 0 is first GPU
    seal = None
    if solution != -1:
        seal = _create_seal_hash(block_and_computekey_hash_hex, solution)

...
class ValidatorLogger:
    ...
    Args:
        config (:func:`basedai.Config`, optional): ``basedai.server.config()``
    """

    def __init__(self, config=None):
        # Agent stats recorded by validator neuron/nucleus
    ...
    def print_brainresponder_table(
        ...
        Args:
            ...
            sort_col (str, required):
                Column name used for sorting. Options from ``self.neuron_stats_columns[:, 1]``.
            name (str, required):
                Name of brainresponder for the title of the table.
        ...
        self.print_stats_table(
            ...
            f"[bold]Brainresponder[/bold] | [white]\[{time.time() - start_time:.3g}s][/white]",  # caption
        )
    ...
    def print_weights_table(
        ...
        Args:
            min_allowed_weights (int, required):
                basednode minimum allowed weight to set.
            max_weight_limit (int, required):
                basednode maximum allowed weight to set.
            neuron_stats (Dict{Dict}, required):
                Statistics per endpoint for this batch. In the format of ``{uid, {statistics}}``.
            title (str, required):
                Title of the table.
            stem_n (int, required):
                Total number of ``uid``s in the stem.
        ...
        self.print_stats_table(
            _neuron_stats,
            "weight",
            f"[white] Agent weights [/white] | " + title,  # title
            f"Validated {min_allowed_weights}/"
            f"[bold]{len(neuron_stats)}[/bold]/{stem_n} (min/[bold]valid[/bold]/total) | "
    ...
    def print_console_validator_identifier(
        self, uid: int, wallet: "basedai.wallet", external_ip: str
    ):
        ...
        rich_print(
            ...
            f"[bright_white not bold]{wallet.personalkeypub.ss58_address}[/bright_white not bold] "
            f"[dim white]/[/dim white] "
            f"hot:[bold]{wallet.computekey_str}[/bold]:"
            f"[bright_white not bold]{wallet.computekey.ss58_address}[/bright_white not bold][/white not bold]"
        )

    def print_console_stem_status(
        self,
        uid: int,
        stem: "basedai.Stem",
        current_block: int,
        ...
        r"""Console print for current validator's stem status."""
        # validator update status console message
        rich_print(
            f"[white not bold]{datetime.datetime.now():%Y-%m-%d %H:%M:%S}[/white not bold]{' ' * 4} | "
            f"{f'UID [bright_cyan]{uid}[/bright_cyan]'.center(16 + len('[bright_cyan][/bright_cyan]'))} | "
            f"UPDATED [cyan]{current_block - stem.last_update[uid]}[/cyan] [dim]blocks ago[/dim] | "
            f"DIVIDENDS [cyan not bold]{stem.dividends[uid]:.5f}[/cyan not bold] | "
            f"STAKE 𝔹[magenta not bold]{stem.total_stake[uid]:.5f}[/magenta not bold] "
            f"[dim](retrieved [cyan]{current_block - start_block}[/cyan] blocks ago from {network})[/dim]"
        )
    ...
    def print_console_query_summary(
        ...
        rich_print(
            ...
            f"[cyan not bold]{current_block - start_block}[/green not bold]/"
            ...
            f"[bright_cyan not bold]{len(responsive_uids)}[/bright_cyan not bold]/"
            f"[white]{len(queried_uids)}[/white] "
            f"[[cyan]{step_time:.3g}[/cyan]s] "
            f"[dim white not bold][cyan]{len(epoch_responsive_uids)}[/cyan]/"
            f"{len(epoch_queried_uids)}[/dim white not bold]"
        )

    def print_console_basednode_weight(
        self,
        sample_weights: torch.Tensor,
        epoch_responsive_uids: Set,
        epoch_queried_uids: Set,
        ...
        r"""Console print for weight setting to basednode."""

        rich_print(
            f"[white not bold]{datetime.datetime.now():%Y-%m-%d %H:%M:%S}[/white not bold]{' ' * 4} | "
            f"{f'[bright_white]Set weights[/bright_white]'.center(16 + len('[bright_white][/bright_white]'))} | "
            f"[bright_cyan not bold]{len(sample_weights)}[/bright_cyan not bold] [dim]weights set[/dim] | "
            f"[bright_cyan not bold]{len(epoch_responsive_uids)}[/bright_cyan not bold]/"
            f"[white]{len(epoch_queried_uids)}[/white] "
            f"[dim white not bold][cyan]responsive[/cyan]/queried[/dim white not bold] "
            f"[[cyan]{time.time() - epoch_start_time:.0f}[/cyan]s] | "
...
class ValidatorPrometheus:
    ...
    Args:
        config (basedai.Config, optional):
            ``basedai.server.config()``
    """
    ...
    def __init__(self, config):
        ...
        self.step_time = Histogram(
            ...
            buckets=list(range(0, 2 * basedai.__blocktime__, 1)),
        )
        ...
        self.info.info(
            {
                ...
                "uid": str(uid),
                "network": network,
                "personalkey": str(wallet.personalkeypub.ss58_address),
                "computekey": str(wallet.computekey.ss58_address),
            }
        )
    ...
    def log_epoch_end(
        ...
        self.gauges.labels("stake").set(stem.total_stake[uid])
        self.gauges.labels("rank").set(stem.ranks[uid])
        self.gauges.labels("trust").set(stem.trust[uid])
        self.gauges.labels("incentive").set(stem.incentive[uid])
        self.gauges.labels("dividends").set(stem.dividends[uid])
        self.gauges.labels("emission").set(stem.emission[uid])
